...
# =========================
# GEMINI CONFIG (MOTORE CORRETTO)
# =========================

genai.configure(api_key=st.secrets["API_KEY"])
# Sostituito gemini-2.5-flash con gemini-1.5-flash (modello ufficiale disponibile)
...
def build_prompt(symbol, timeframe, account_size, risk_percent, prop_mode, ai_profile):
    ...
    profile_rule = "Profilo Standard: Approccio bilanciato. Controlla trend, RSI, struttura e RR."
    # (Logica profili invariata)
    return f"""
Sei ProTrade AI, assistente professionale di analisi trading creato per Andreas De Marco.
Analizza il grafico caricato come un trader umano esperto. Non inventare dati.

DATI OPERATIVI:
Strumento: {symbol}, Timeframe: {timeframe}, Account: {account_size} USD, Rischio: {risk_percent}%

REGOLE:
- LONG/SHORT se il setup è buono.
- ASPETTA se manca conferma.
- NO TRADE se mercato laterale o RR scarso.

Rispondi ESATTAMENTE con questo formato:
VERDETTO: [LONG/SHORT/ASPETTA/NO TRADE]
TIPO SEGNALE: [AGGRESSIVO/STANDARD/CONSERVATIVO]
TRADE SCORE: [0-100]
CONFIDENCE: [0-100%]
MARKET BIAS: [Trend/Struttura/Momentum/Volatilità]
RSI CHECK: [Valore/Condizione]
TRADE SETUP: [Entry/SL/TP1/TP2/TP3/RR]
RISK MANAGER: [Size/Breakeven/Parziale]
...
# UI (INVARIATA)
# =========================
# (Tutto il resto del tuo codice UI rimane esattamente uguale)
